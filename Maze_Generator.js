/* globals Cell, MazeGraphic, Point */
/* exported maze, numColumns, numRows, showProgress, squareSize */
const squareSize = 20;
const showProgress = true; //for toggling console output while creating the maze
const difficulty = 0.5; //literally the fraction of cells generated by random walk


let numRows, numColumns;
const maze = []; //will be a 2D array
let currentCell;
const visitedCellsStack = [];
let doneGenerating = false;
const mazeGraphic = new MazeGraphic();
let randomWalk = true;


function setup() {
  createCanvas(400, 400);
  background(0);
  stroke(255);
  noFill();
  if (!squareSize || squareSize > width || squareSize > height) {
    console.log('Error: square size is too small.');
    doneGenerating = true;
    noLoop();
    return;
  }

  numRows = floor(height / squareSize);
  numColumns = floor(width / squareSize);
  for (let row = 0; row < numRows; row++) {
    maze[row] = [];
    for (let column = 0; column < numColumns; column++) {
      maze[row].push(new Cell(column, row));
    }
  }

  currentCell = maze[int(random(numRows))][int(random(numColumns))];
  currentCell.visited = true;
  visitedCellsStack.push(new Point(currentCell.point.x, currentCell.point.y));
  if (visitedCellsStack.length === numRows * numColumns) {
    doneGenerating = true;
    MazeGraphic();
  }
}


function draw() {
  if (!doneGenerating) {
    if (showProgress) {
      rect(0, 0, numColumns * squareSize, numRows * squareSize);
      for (const row of maze) {
        for (const cell of row) {
          cell.show();
        }
      }
      console.log(frameRate().toFixed(1), 'FPS');
      console.log(`${millis().toFixed(0) / 1000}s`);
      console.log(`${visitedCellsStack.length} cells`);
      console.log('Randomly walking:', randomWalk);
    }

    if (!randomWalk) { //depth-first with backtracking maze generation (easy-maze generation)
      const nextCell = currentCell.getRandomNeighbor(false);
      if (nextCell === undefined) {
        if (visitedCellsStack.length) {
          const point = visitedCellsStack.pop();
          currentCell = maze[point.y][point.x];
        } else {
          doneGenerating = true;
          mazeGraphic.generate();
        }
      } else {
        if (!nextCell.visited) {
          if (currentCell.point.y - nextCell.point.y === 1) {
            currentCell.topWall = false;
          } else if (currentCell.point.y - nextCell.point.y === -1) {
            nextCell.topWall = false;
          } else if (currentCell.point.x - nextCell.point.x === 1) {
            currentCell.leftWall = false;
          } else if (currentCell.point.x - nextCell.point.x === -1) {
            nextCell.leftWall = false;
          }
        }
        visitedCellsStack.push(new Point(nextCell.point.x, nextCell.point.y));
        if (visitedCellsStack.length === numRows * numColumns) {
          doneGenerating = true;
          mazeGraphic.generate();
        }
        currentCell = nextCell;
      }
    } else { //random-walk maze generation (hard-maze generation)
      const nextCell = currentCell.getRandomNeighbor(null);
      if (!nextCell.visited) {
        if (currentCell.point.y - nextCell.point.y === 1) {
          currentCell.topWall = false;
        } else if (currentCell.point.y - nextCell.point.y === -1) {
          nextCell.topWall = false;
        } else if (currentCell.point.x - nextCell.point.x === 1) {
          currentCell.leftWall = false;
        } else if (currentCell.point.x - nextCell.point.x === -1) {
          nextCell.leftWall = false;
        }
        visitedCellsStack.push(new Point(nextCell.point.x, nextCell.point.y));
        if (visitedCellsStack.length >= difficulty * numRows * numColumns) {
          randomWalk = false;
        }
        if (visitedCellsStack.length === numRows * numColumns) {
          doneGenerating = true;
          mazeGraphic.generate();
        }
      }
      currentCell = nextCell;
    }
    currentCell.visited = true;
  } else {
    background(0);
    mazeGraphic.show();
    console.log('Done!');
    noLoop();
  }
}